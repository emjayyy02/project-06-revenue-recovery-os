...
(async () => {
 ...
 const go = async path => { await page.goto(`http://127.0.0.1:5175${path}`); await page.locator('main h1').waitFor(); await page.waitForFunction(() => !document.querySelector('main [aria-busy="true"]')); };
 ...
 for (const width of [320,768,1440]) {
   ...
   for (const path of ['/customers','/approvals','/dashboard','/analytics','/customers/account-3']) {
     ...
     if (['/customers','/approvals'].includes(path) && width !== 768) await page.screenshot({path:`screenshots/m10-${path.slice(1)}-${width}.png`,fullPage:width===320});
   }
 }
 ...
 assert((await page.locator('.customer-table tbody tr').first().innerText()).includes('UNAVAILABLE'));
 ...
 assert((await page.locator('.customer-table tbody tr').first().innerText()).includes('LOW'));
 ...
 for(const path of ['/customers','/approvals']) {
   ...
   delay=600; await page.goto(`http://127.0.0.1:5175${path}`); await page.getByText(path==='/customers'?'Loading customers…':'Loading approvals…',{exact:true}).waitFor(); delay=0;
   await page.waitForFunction(()=>!document.querySelector('main [aria-busy="true"]')); if(path==='/customers') await readyCustomers();
 }
 ...
 fs.writeFileSync('screenshots/m10-browser-results.json',JSON.stringify({scope:'Isolated HTTP fixtures; no external mutations',checks:results},null,2));
 await browser.close();
